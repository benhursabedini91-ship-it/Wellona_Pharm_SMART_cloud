const MODULES = [
  { id: 'orders', label: 'Porosi AI', icon: '🧠' },
  { id: 'invoices', label: 'Faktura AI', icon: '📄' },
  { id: 'bank', label: 'Banka AI', icon: '🏦' },
  { id: 'analyst', label: 'Analyst', icon: '📊' },
  { id: 'loyalty', label: 'Loyalty', icon: '🎟️' },
    ...
    <div class="space-y-4">
      <div class="grid grid-cols-4 gap-3">
        <div class="p-5 bg-white rounded-2xl shadow-sm">
          <div class="text-xs text-neutral-500">🔥 Kritik</div>
          <div class="text-3xl font-semibold">${krit}</div>
        </div>
        <div class="p-5 bg-white rounded-2xl shadow-sm">
          <div class="text-xs text-neutral-500">⏳ Urgjent</div>
      ...
      <div class="flex gap-2">
        <select id="targetDays" class="px-3 py-2 rounded-xl bg-neutral-100">
          ${[3,6,7,10,14,21,28].map(n => `<option value="${n}" ${n===appState.targetDays?'selected':''}>${n} ditë</option>`).join('')}
        </select>
        <select id="salesWindow" class="px-3 py-2 rounded-xl bg-neutral-100">
          ${[15,30,60].map(n => `<option value="${n}" ${n===appState.salesWindow?'selected':''}>${n} ditë</option>`).join('')}
        </select>
...
async function placeOrder(supplier) {
  ...
  try {
    ...
    alert(`✅ ${supplier} order u gjenerua!`);
  } catch(e) {
